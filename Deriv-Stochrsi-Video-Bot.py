...
parser.add_argument("--min-srsi", type=float,
                    default=float(os.environ.get("FILTER_MIN_SRSI", "0.20")),
                    help="Minimum SRSI at signal time - blocks exhausted signals (default: 0.20)")
parser.add_argument("--min-flat-extreme", type=float,
                    default=float(os.environ.get("FILTER_MIN_FLAT_EXTREME", "0.02")),
                    help="Minimum flat_extreme for LONG signals - blocks rock-bottom flat zones (default: 0.02)")
...
async def process_tick(ws, tick_data, last_trade_time):
    ...
    global _session_pnl, _session_halt_until, _pending_buy
    _tick_count += 1
